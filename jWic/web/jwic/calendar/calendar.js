...
//	var	monthName =	new	Array("January","February","March","April","May","June","July","August","September","October","November","December")
    ...
    */
	...
	function constructDate(d,m,y)
	{
		...
		sTmp = sTmp.replace	("<o>",jWic().calendar.monthName[m])
   ...
   function WeekNbr(n) {
	...
	function constructCalendar () {
		...
		if (showWeekNumber==1)
		{
			sHTML += "<td width=27><b>" + jWic().calendar.weekString + "</b></td><td width=1 rowspan=7 bgcolor='#d0d0d0' style='padding:0px'><img src='"+imgDir+"divider.gif' width=1></td>"
		}

		for	(i=0; i<7; i++)	{
			sHTML += "<td width='27' align='right'><B>"+ jWic().calendar.dayName[i]+"</B></td>"
		}
		sHTML +="</tr><tr>"
		...
		for	( datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
		{
			...
			dateMessage = "onmousemove='window.status=\""+jWic().calendar.selectDateMessage.replace("[date]",constructDate(datePointer,monthSelected,yearSelected))+"\"' onmouseout='window.status=\"\"' "
		...
		frmDocument.getElementById("spanMonth").innerHTML = "&nbsp;" +	jWic().calendar.monthName[monthSelected] + "&nbsp;<IMG id='changeMonth' SRC='"+imgDir+"drop1.gif' WIDTH='12' HEIGHT='10' BORDER=0>"
	...
	function popUpCalendar(ctl,	ctl2, format) {
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
				...
				if ( formatChar	!= "" )
				{
					...
					for	(i=0;i<3;i++)
					{
						...
						else if	(aFormat[i]=="mmm")
						{
							for	(j=0; j<12;	j++)
							{
								if (aData[i]==jWic().calendar.monthName[j])
								{
									monthSelected=j
									tokensChanged ++
								}
				...
				var maxY = wnSize[1] + wnSize[3] - 150;
				if (y > maxY) y = maxY;
				if (y < 1) { y = 1 }
				...
				/*if (Effect) {
					Effect.Appear('calendar', { from: 0.0, to: 1.0});
				} else {*/
	...
	function initLib(newDocument) {
		...
		if (startAt!=0)
		{
			var dayName = jWic().calendar.dayName;
			dayName = new Array	(dayName[1],
					dayName[2],
					dayName[3],
					dayName[4],
					dayName[5],
